...
var DEFAULT_URL = 'file/sample.pdf';
  ...
  (function (root, factory) {
   ...
   var PDFViewerApplication = {
    ...
    open: function pdfViewOpen(file, args) {
     ...
     if(localStorage.getItem("ngStorage-pdfPrintData")){
        var pdfPrintData = localStorage.getItem("ngStorage-pdfPrintData");
        var myArray = pdfPrintData.replace("[","").replace("]").split(",");
        for(var i=0; i<myArray.length; i++) { myArray[i] = parseInt(myArray[i], 10); }
        var myData = new Uint8Array(myArray);
        var loadingTask = pdfjsLib.getDocument(myData);
        localStorage.removeItem('ngStorage-pdfPrintData')
     }else{
